 ...
 * @param <T>
 */
public class RandomWeibullTime<T> extends WeibullTime<T> {
     ...
     */
    public RandomWeibullTime(final double mean, final double deviceDeviation, final double networkDeviation, final RandomGenerator random) {
     ...
     *            {@link RandomGenerator} used internally
     */
    public RandomWeibullTime(final double mean, final double deviceDeviation, final double networkDeviation, final Time start, final RandomGenerator random) {
     ...
     *            {@link RandomGenerator} used internally
     */
    public RandomWeibullTime(final double mean, final double deviceDeviation, final double networkDeviation, final double deviationDeviation, final Time start, final RandomGenerator random) {
    ...
    private static double weibullValue(final double mean, final double deviation, final RandomGenerator random) {
    	if (deviation > 0) {
    		WeibullDistribution dist = weibullFromMean(mean, deviation, random);
